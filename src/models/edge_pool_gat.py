...
from torch_geometric.nn import GATConv, EdgePooling, global_mean_pool
from torch.nn import Linear
from torch_geometric.data import Batch

class EdgePoolGAT(torch.nn.Module):
    """
    Scalable Graph Neural Network with Graph Attention Layers (GAT), Edge Pooling,
    ...
    def __init__(self, num_node_features: int, num_classes: int, hidden_dim: int = 64, heads: int = 4):
        super(EdgePoolGAT, self).__init__()

        # GNN layers with Graph Attention mechanism
        self.conv1 = GATConv(num_node_features, hidden_dim, heads=heads, concat=True)
        self.pool1 = EdgePooling(hidden_dim * heads)
        self.conv2 = GATConv(heads * hidden_dim, hidden_dim, heads=heads, concat=True)
        self.pool2 = EdgePooling(hidden_dim * heads)
    ...
    def forward(self, data: Batch) -> torch.Tensor:
        ...
        x, edge_index = self.pool1(x, edge_index)
        x = F.relu(x)
        x = self.conv2(x, edge_index)
        x, edge_index = self.pool2(x, edge_index)
        x = F.relu(x)
